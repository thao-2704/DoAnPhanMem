                ...
                <div class="card-header pb-0 p-3">
                    <div class="row">
                        ...
                        <div class="col-6 text-end">
                            <a href="{{route('chucvu.create')}}" class="btn btn-outline-primary btn-sm mb-0">Thêm Chức Vụ</a>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Tên Chức Vụ</th>
                            ...
                            <tbody>
                                @foreach ($chucvu as $key => $item)
                                <tr>
                                    <td>
                                        <p class="text-xs ms-sm-3 font-weight-bold mb-0">{{$key + 1}}</p>
                                    </td>
                                    <td><p class="text-xs ms-sm-3 font-weight-bold mb-0">{{$item->ten}}</p></td>
                                    <td><p class="text-xs ms-sm-3 font-weight-bold mb-0">{{$item->mota}}</p></td>
                                    <td>
                                        <form class="float-start mt-2" action="{{route('chucvu.destroy',[$item->id])}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                    ...
                                    <td>
                                    <a href="{{route('chucvu.edit',[$item->id])}}" class="btn btn-link text-dark px-3 mx-n2 mb-0">
